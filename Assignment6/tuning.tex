...
We start by doing an set point exciting experiment on the controller \texttt{24\_LC1016}, where we do several step changes in the set point, and then observe the output. We then import this to MATLAB and use the built in function \mcode{n4sid} to estimate a state space realization of the system we are looking at. In \autoref{fig:lc1016_identified} we see the actual response of the controller  normalized and compared to the identified model. We see that the identified model is fairly good.
