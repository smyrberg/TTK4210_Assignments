...
figure(4)
    subplot 211
    y=y_KFLQR_antiwindup;
    plot(y_ref.time,y_ref.signals.values(:,1),'r')
    title('LQG (Kalman+LQR) controller with integral  and anti-windup, y_1');
    hold on
    plot(y.time,y.signals.values(:,1),'b')
    ylim([-3 3])
    ...
    subplot 212
    plot(y_ref.time,y_ref.signals.values(:,2),'r')
    title('LQG (Kalman+LQR) controller with integral action and anti-windup, y_2');
    hold on
    plot(y.time,y.signals.values(:,2),'b')
    ylim([-3 3])
